...
export class MarketDataService {
   ...
   */
  static async getIndexHistoricalData(indexName: string): Promise<StockHistoricalData[]> {
    // In the future, replace with API call
    // return fetch(`/api/index-history/${indexName}`).then(res => res.json());
    return Promise.resolve(
      mockIndexHistoricalData[indexName as keyof typeof mockIndexHistoricalData] || 
   ...
   */
  static formatChartData(historicalData: StockHistoricalData[]): ChartDataItem[] {
   ...
   */
  static async getStockHistoricalData(stock: StockData): Promise<StockHistoricalData[]> {
    // In the future, replace with API call
